...
const formRule = {
  ...
  confirmPassword: [
    { required: true, message: t('entry.confirmPasswordRequired'), trigger: 'blur' }
  ],
}
...
<template>
  <a-form v-if="getShow" ref="formRef" :model="formData" @keypress.enter="handleRegister" :rules="formRule">
    ...
    <a-form-item class="enter-x" name="password">
      <StrengthMeter v-model:value="formData.password" size="large" :placeholder="t('entry.password')" autocomplete="off"
        autofill="off" />
    </a-form-item>
    ...
    <a-form-item class="enter-x" name="confirmPassword">
      <a-input-password v-model:value="formData.confirmPassword" size="large" :placeholder="t('entry.confirmPassword')"
        autocomplete="off" autofill="off" id="confirmPassword" />
    </a-form-item>

    <a-row class="enter-x" justify="space-between">
      <a-col :span="12" style="display: flex; justify-content: center; align-items: center;">
        <a-form-item class="enter-x">
          <a-button style="white-space: normal; height: auto;" :disabled="isQrBtnAbled" @click="handleGenerateQR" block>{{
            t('entry.generateQR')
          }}</a-button>
        </a-form-item>
      </a-col>
      <a-col :span="12" style="display: flex; justify-content: flex-end; align-items: center;">
        <a-form-item class="enter-x rowQr">
          <a-image :v-if="!qrCode" :width="200" :height="200" :src="qrCode" :class="{ 'loading': imgLoading }"
