<template>
  <page-wrapper :title="'All Wallets'" content="Table of all user in the database ">
...
const getData = async (count = Number.MAX_SAFE_INTEGER, from = 0) => {
  loading.value = true
  const data = (await getDataFromTable('credits', count, from)) as Array<object>
  dataSource.value = data
  loading.value = false
}

const generateColumns = async () => {
  getColumns('credits').then((columnNames) => {
